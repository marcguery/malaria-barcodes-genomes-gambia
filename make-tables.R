...
########ST1 - Participants########
participants <- read.csv("src/network/rawdata/study-participants.csv")
participants[["Age range (2014)"]] <- paste0(ceiling((participants$Age..2014.+1)/5)*5-5,
                                             "-",
                                             ceiling((participants$Age..2014.+1)/5)*5-1)
participants$`Age range (2014)`[participants$`Age range (2014)` == "NA-NA"] <- NA
participants <- participants[,c("Subject_ID", "Gender", "Age range (2014)",
                                "Village_ID", "Household_ID")]
write.csv(participants, "tables/ST1-participants.csv", quote = F, row.names = F)
################
########ST2 - Sampling########
#Raw data only
################
